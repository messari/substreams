...
use crate::utils::{BigIntSerializeExt, get_latest_day, get_latest_hour};
...
impl StatsRetriever {
    ...
    pub(crate) fn get_total_stats(&self, variable_key: StoreKey, count_key: StoreKey) -> Stats {
        let count = self.aggregation_retriever.get_total_sum(count_key);

        let (sum, sum_squares) = self.aggregation_retriever.get_total_stats_values(variable_key.clone());
        let mean = sum.clone() / count.clone();
        let variance = (sum.pow(2) - sum_squares) / count.clone();

        Stats {
            id: self.entity_id.clone(),
            count: Some(count.into()),
            mean: Some(mean.into()),
            max: Some(self.max_value_retriever.get_value(variable_key.clone()).into()),
            min: Some(self.min_value_retriever.get_value(variable_key).into()),
            sum: Some(sum.into()),
