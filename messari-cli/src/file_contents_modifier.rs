...
pub(crate) fn safely_modify_file_contents(file_contents_modifications: Vec<FileContentsModification>) {
    ...
    for file_contents_modification in file_contents_modifications {
        let reverse_operation = match file_contents_modification.get_reverse_operation() {
            ...
            Err(error) => {
                if !reverse_operations.is_empty() {
                    ...
                    println!("Failed to reverse operation: {}!\nError: {}\n", file_contents_modification, error);
        ...
        if let Err(error) = file_contents_modification.apply_operation() {
            if !reverse_operations.is_empty() {
                ...
                println!("Failed to apply operation: {}!\nError: {}\n", file_contents_modification, error);
                rollback_file_contents_changes(reverse_operations);
                return;
            }
        }
...
fn rollback_file_contents_changes(rollback_operations: Vec<FileContentsModification>) {
    let mut rollback_operations_iter = rollback_operations.into_iter();
    for rollback_operation in rollback_operations_iter {
        if let Err(error) = rollback_operation.apply_operation() {
            ...
            panic!("Failed to rollback operation: {}\nError:{}\n\nManual rollback command:\nmessari rollback TODO..", rollback_operation, error);
        }
    }
...
impl Display for FileContentsModification {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            FileContentsModification::CreateFile(file) => write!(f, "CreateFile({})", file.filepath.to_string_lossy()),
            FileContentsModification::UpdateFile(file) => write!(f, "UpdateFile({})", file.filepath.to_string_lossy()),
            FileContentsModification::DeleteFile(filepath) => write!(f, "DeleteFile({})", filepath.to_string_lossy()),
            FileContentsModification::CreateFolder(folder_path) => write!(f, "CreateFolder({})", folder_path.to_string_lossy()),
            FileContentsModification::DeleteFolder(folder_path) => write!(f, "DeleteFolder({})", folder_path.to_string_lossy()),
        }
    }
}
...
impl FileContentsModification {
    fn get_reverse_operation(&self) -> Result<FileContentsModification, std::io::Error> {
        match self {
            FileContentsModification::CreateFile(file) => Ok(FileContentsModification::DeleteFile(file.filepath.clone())),
            FileContentsModification::UpdateFile(file) => {
                let current_file_contents = fs::read_to_string(&file.filepath)?;
                Ok(FileContentsModification::UpdateFile(File {filepath: file.filepath.clone(), file_contents: current_file_contents}))
            }
            FileContentsModification::DeleteFile(filepath) => {
                let current_file_contents = fs::read_to_string(filepath)?;
                Ok(FileContentsModification::CreateFile(File {filepath: file.filepath.clone(), file_contents: current_file_contents}))
            }
            FileContentsModification::CreateFolder(folder_path) => Ok(FileContentsModification::DeleteFolder(folder_path.clone())),
            FileContentsModification::DeleteFolder(folder_path) => Ok(FileContentsModification::DeleteFolder(folder_path.clone())),
        }
    }
    ...
    fn apply_operation(self) -> std::io::Result<()> {
        match self {
            FileContentsModification::CreateFile(file) => fs::write(file.filepath, file.file_contents),
            FileContentsModification::UpdateFile(file) => fs::write(file.filepath, file.file_contents),
