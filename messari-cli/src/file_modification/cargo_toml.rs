...
impl CargoToml {
            ...
            match dependency.location {
                Location::Local { local_path } => {
                    let relative_path = get_relative_path(
                        &self.cargo_dir,
                        &get_repo_root_folder().join(local_path),
                    );
                    let workspace_dependency = Manifest::from_str(&format!(
                        "{} = {{ path = \"{}\" }}",
                        dependency.crate_name, relative_path
                    ))
                    .unwrap()
                    .data
                ...
                Location::Remote {
                    major_version_requirement,
                } => {
                    let dependency_item =
                        get_dependency(dependency.crate_name, major_version_requirement)
                            .to_toml(&self.cargo_dir);
    ...
    fn create_section(&mut self, section: &str, first_section_part: &str) {
        let manifest_section = Manifest::from_str(&format!("\n[{}]\n", section))
            .unwrap()
            .data
            .get(first_section_part)
            .unwrap()
            .clone();
        self.manifest
            .data
            .insert(first_section_part, manifest_section);
    }
}
