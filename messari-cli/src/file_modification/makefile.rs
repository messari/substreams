...
impl MakeFile {
    ...
    pub(crate) fn load_from_file(makefile_filepath: &PathBuf) -> Self {
        ...
        let mut makefile_contents = fs::read_to_string(makefile_filepath).expect(&format!(
            "Unable to read Makefile contents! Filepath: {}",
            makefile_filepath.to_string_lossy()
        ));
        makefile_contents = makefile_contents.replace("\r\n", "\n").to_string();
        makefile_contents = re_newline_spaces
            .replace_all(&makefile_contents, "\n")
            .to_string();
        makefile_contents = re_start_new_lines
            .replace(&makefile_contents, "")
            .to_string();
        makefile_contents = re_end_new_lines.replace(&makefile_contents, "").to_string();
        makefile_contents = re_command_block_gaps
            .replace_all(&makefile_contents, "\n\n")
            .to_string();

        let commands = makefile_contents
            .split("\n\n")
            .map(|block_str| parse_block(block_str, &makefile_filepath))
            .collect();
        ...
        for command in self.commands.iter_mut() {
            if command.command_name == "build-all".to_string() {
                ...
                return if build_all_projects.contains(&project) {
                    ...
                    command
                        .operations
                        .push(format!("$(MAKE) -C {} build", project));
                    true
                };
            }
        }
...
fn parse_block(block_str: &str, makefile_filepath: &PathBuf) -> MakeCommand {
    ...
    let second_line = lines.next().expect(&format!(
        "There was no second line for command block!\nCommand block: {}\nMakefile filepath: {}",
        block_str,
        makefile_filepath.to_string_lossy()
    ));
    ...
    let operations = lines
        .map(|line_str| parse_operation(line_str, block_str, makefile_filepath))
        .collect();
