...
pub fn create_deposit_entity(
    ...
    row
        ...
        .set("gasUsed", &pruned_transaction.gas_used.clone().unwrap())
        .set("gasPrice", &pruned_transaction.gas_price.clone().unwrap())
        .set("logIndex", &deposit_entity_creation.log_index.clone().unwrap())
        ...
        .set("blockNumber", BigInt::from(*block_number))
        .set("timestamp", BigInt::from(*timestamp))
        .set("liquidity", &deposit_entity_creation.liquidity.clone().unwrap())
        .set("inputTokens", &deposit_entity_creation.input_tokens)
        .set("inputTokenAmounts", &deposit_entity_creation.input_token_amounts)
        .set("amountUSD", constants::BIGDECIMAL_ZERO.clone());

    match (&deposit_entity_creation.tick_lower, &deposit_entity_creation.tick_upper) {
...
pub fn create_withdraw_entity(
    ...
    row
        ...
        .set("gasUsed", &pruned_transaction.gas_used.clone().unwrap())
        .set("gasPrice", &pruned_transaction.gas_price.clone().unwrap())
        .set("logIndex", &withdraw_entity_creation.log_index.clone().unwrap())
        ...
        .set("blockNumber", BigInt::from(*block_number))
        .set("timestamp", BigInt::from(*timestamp))
        .set("liquidity", &withdraw_entity_creation.liquidity.clone().unwrap())
        .set("inputTokens", &withdraw_entity_creation.input_tokens)
        .set("inputTokenAmounts", &withdraw_entity_creation.input_token_amounts)
        .set("amountUSD", constants::BIGDECIMAL_ZERO.clone());

    match (&withdraw_entity_creation.tick_lower, &withdraw_entity_creation.tick_upper) {
        (Some(tick_lower), Some(tick_upper)) => {
            row
                .set("tickLower", tick_lower)
                .set("tickUpper", tick_upper);
