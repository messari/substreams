...
pub fn create_dex_amm_protocol_entity(
    ...
    tables
        .create_row("DexAmmProtocol", std::str::from_utf8(entity_id).unwrap())
        .set("name", &dex_amm_protocol_entity_creation.name)
        .set("slug", &dex_amm_protocol_entity_creation.slug)
        .set(
            "schemaVersion",
            &dex_amm_protocol_entity_creation.schema_version,
        )
        .set(
            "substreamVersion",
            &dex_amm_protocol_entity_creation.substream_version,
        )
        .set(
            "methodologyVersion",
            &dex_amm_protocol_entity_creation.methodology_version,
        )
