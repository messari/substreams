...
pub fn create_store_operations_l1_decrease_liquidity(
    ...
    store_operation_factory.track_position_mutation(keys::get_position_key(
        &Hex(&NULL_ADDRESS).to_string(),
        &decrease_liquidity_event.token_id.to_string(),
    ));
}
...
pub fn prepare_decrease_liquidity_entity_changes(
    ...
    let position_id = &keys::get_position_key(
        &Hex(&NULL_ADDRESS).to_string(),
        &decrease_liquidity_event.token_id.to_string(),
    );
    ...
    entity_update_factory
        .store_operations
        .add_position_liquidity(0, &position_id, decrease_liquidity_event.liquidity);
    entity_update_factory
        .store_operations
        .increment_position_withdraw_count(0, &position_id);
    entity_update_factory
        .store_operations
        .add_position_cumulative_withdraw_token_amounts(
            0,
            &position_id,
            vec![
                decrease_liquidity_event.amount0.clone(),
                decrease_liquidity_event.amount1.clone(),
            ],
        );
}
