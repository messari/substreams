...
use substreams::store::{DeltaBigDecimal, DeltaInt64, DeltaBigInt, StoreGet, StoreGetArray, Delta};
...
use crate::schema_lib::dex_amm::v_3_0_3::map::map_dex_amm_v_3_0_3_updates;
use crate::constants;
        ...
        match (index, array_size) {
            ...
            (Some(_), Some(array_size)) => {
                ...
                for i in 0..array_size {
                    ...
                    if let Some(delta) = add_bigint_store_deltas.deltas.iter().find(|d| d.key == key) {
                        value_list[i] = delta.new_value.clone();
                    } else {
    ...
    for delta in &add_bigdecimal_store_deltas.deltas {
        ...
        match (index, array_size) {
            ...
            (Some(_), Some(array_size)) => {
                ...
                for i in 0..array_size {
                    let key = [key_list[0], key_list[1], key_list[2], key_list[3], i.to_string().as_str(), array_size.to_string().as_str()].join(":");
                    if let Some(delta) = add_bigdecimal_store_deltas.deltas.iter().find(|d| d.key == key) {
                        value_list[i] = delta.new_value.clone();
                    } else {
                        panic!("Missing delta for key: {}", key)
                    }
