...
fn via_chainlink_feed_registry(network_config: &NetworkConfig,
                               token_address: Vec<u8>) -> Option<BigDecimal> {
    ...
    if let (Some(price_mantissa), Some(decimals)) = (price_mantissa_res, decimals_res) {
        Some(BigDecimal::from(price_mantissa.1).div(math::exponent_to_big_decimal(decimals.as_u64() as u8)))
    } else {
        None
    }
}
