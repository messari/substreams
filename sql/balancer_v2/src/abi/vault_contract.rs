                    ...
                    &[
                            ...
                            let v = self
                                ...
                                .map(|inner| ethabi::Token::Tuple(
                                    vec![
                                        ...
                                        (num_bigint::Sign::Minus, _) => {
                                        panic!("negative numbers are not supported") }, }
                                        .as_slice(),),), ethabi::Token::Bytes(inner.4.clone())
                                    ],
                                ))
                                .collect();
                        ...
                        ethabi::Token::Tuple(
                            vec![
                                ...
                                .funds.0)), ethabi::Token::Bool(self.funds.1.clone()),
                                ethabi::Token::Address(ethabi::Address::from_slice(& self
                                .funds.2)), ethabi::Token::Bool(self.funds.3.clone())
                            ],
                        ),
                        {
                            let v = self
            ...
            pub fn encode(&self) -> Vec<u8> {
                let data = ethabi::encode(
                    &[
                        ethabi::Token::Address(ethabi::Address::from_slice(&self.user)),
                        ethabi::Token::Address(
                            ethabi::Address::from_slice(&self.relayer),
                        ),
                    ],
                );
                let mut encoded = Vec::with_capacity(4 + data.len());
                            ...
                            let v = self
                                ...
                                .map(|inner| ethabi::Token::Tuple(
                                    vec![
                                        ...
                                        .as_slice(),),),
                                        ethabi::Token::Address(ethabi::Address::from_slice(& inner
                                        .3)), ethabi::Token::Address(ethabi::Address::from_slice(&
                                        inner.4))
                                    ],
                                ))
                                .collect();
            ...
            pub fn encode(&self) -> Vec<u8> {
                let data = ethabi::encode(
                    &[
                        ethabi::Token::Uint(
                            ethabi::Uint::from_big_endian(
                                match self.specialization.clone().to_bytes_be() {
                                    (num_bigint::Sign::Plus, bytes) => bytes,
                                    (num_bigint::Sign::NoSign, bytes) => bytes,
                                    (num_bigint::Sign::Minus, _) => {
                                        panic!("negative numbers are not supported")
        ...
        impl SetAuthorizer {
            ...
            pub fn encode(&self) -> Vec<u8> {
                let data = ethabi::encode(
                    &[
                        ethabi::Token::Address(
                            ethabi::Address::from_slice(&self.new_authorizer),
                        ),
                    ],
                );
                let mut encoded = Vec::with_capacity(4 + data.len());
            ...
            pub fn encode(&self) -> Vec<u8> {
                let data = ethabi::encode(
                    &[
                        ethabi::Token::Tuple(
                            vec![
                                ethabi::Token::FixedBytes(self.single_swap.0.as_ref()
                                .to_vec()),
                                ethabi::Token::Uint(ethabi::Uint::from_big_endian(match self
                                .single_swap.1.clone().to_bytes_be() {
                                (num_bigint::Sign::Plus, bytes) => bytes,
                                (num_bigint::Sign::NoSign, bytes) => bytes,
                                (num_bigint::Sign::Minus, _) => {
                                panic!("negative numbers are not supported") }, }
                                ...
                                .single_swap.3)),
                                ethabi::Token::Uint(ethabi::Uint::from_big_endian(match self
                                .single_swap.4.clone().to_bytes_be() {
                                (num_bigint::Sign::Plus, bytes) => bytes,
                                (num_bigint::Sign::NoSign, bytes) => bytes,
                                (num_bigint::Sign::Minus, _) => {
                                panic!("negative numbers are not supported") }, }
                                .as_slice(),),), ethabi::Token::Bytes(self.single_swap.5
                                .clone())
                            ],
                        ),
                        ethabi::Token::Tuple(
